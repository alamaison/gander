...
/**
 * Test the import statements load the expected modules and packages.
 * 
 * Doesn't check that they are bound to the right names or anything else beyond
 * loading.
 */
public class ImportLoadingTest extends AbstractModelTest {
    ...
    public void fromDistantImport() throws Throwable {
        ...
        assertKeys("Loading a module that imports something from another "
                ...
                getModel().getTopLevel().getModules(),
                addToBuiltins("relative", "sibling"));
    }
    ...
    public void siblingPackageImport() throws Throwable {
        ...
        assertKeys("Loading a module that imports a sibling must add "
                ...
                getModel().getTopLevel().getModules(),
                addToBuiltins(start, "sibling_package"));
    }
    ...
    public void nestedCircularImport() throws Throwable {
        ...
        assertKeys("Loading a package must also load any modules it "
                ...
                .getModules(),
                addToBuiltins("nested_circular1", "nested_circular2"));
    ...
    public void explicitRelativeImport() throws Throwable {
        String start = "relative_explicit.import_explicit_relative";

        getModel().loadModule(start);

        assertKeys("Explicit relative import didn't load the expected module",
                getModel().lookup("relative_explicit").getModules(),
                "import_explicit_relative", "ted");
    }

    @Test
    public void explicitRelativeImportDown() throws Throwable {
        String start = "relative_explicit.import_explicit_relative_down";

        getModel().loadModule(start);

        assertKeys("Explicit relative import must load any parent packages",
                getModel().lookup("relative_explicit").getModules(),
                "import_explicit_relative_down", "ted");

        assertKeys("Explicit relative import didn't load the expected module",
                getModel().lookup("relative_explicit.ted").getModules(),
                "eileen");
    }

    @Test
    public void explicitRelativeImportUp() throws Throwable {
        String start = "relative_explicit.ted.import_explicit_relative_up";

        getModel().loadModule(start);

        assertKeys("Explicit relative import didn't load the expected modules",
                getModel().lookup("relative_explicit").getModules(), "ted",
                "frances");
    }

    @Test
    public void explicitRelativeImportUpThenDown() throws Throwable {
        String start = "relative_explicit.ted.import_explicit_relative_up_then_down";

        getModel().loadModule(start);

        assertKeys("Explicit relative import didn't load the parent packages",
                getModel().lookup("relative_explicit").getModules(), "ted",
                "frances");

        assertKeys("Explicit relative import didn't load the expected modules",
                getModel().lookup("relative_explicit.frances").getModules(),
                "rhys");
    }
}
